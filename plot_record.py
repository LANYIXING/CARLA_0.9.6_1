...
def plt_reward(
    ...
    time = datetime.datetime.now()
    statistical_data = rank_and_average(GLOBAL_RUNNING_R)
    with open(title+'.csv', 'a', newline='') as myFile:
        Writer = csv.writer(myFile, dialect='excel')
        Writer.writerow([title] + [str(time)])
        Writer.writerow(GLOBAL_RUNNING_R)
        Writer.writerow(statistical_data)
...
if __name__ == '__main__':
    with open('A3C_TEST_RECORD.csv', 'r') as csvfile:
        ...
        for i, rows in enumerate(reader2):
            ...
            if i == 75:
                rows2 = np.array(rows)
    statistical_data = np.stack((rows1, rows2), axis=0)

    plot_error_bar(data=statistical_data)
