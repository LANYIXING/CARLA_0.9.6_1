    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point1 = Transform(
                Location(
                    x=250, y=129.750000, z=1.320625), Rotation(
            ...
            spawn_point2 = Transform(
                Location(
                    x=300, y=129.750000, z=1.320625), Rotation(
                    pitch=0.000000, yaw=179.999756, roll=0.000000))
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if not self._show_info:
            return
        t = world.player.get_transform()  # get_transform
...
def game_loop(args):
    ...
    try:
        ...
        # the input of this class is world of "Town01", hud, and the
        # actor(vehicle defined in main args.)
        # client.get_world: return "Town01"
        world = World(client.get_world(), hud, args.filter)
        # controller = KeyboardControl(world, True)
        controller = RLControl(world, False)
        ...
        while True:
            ...
            # print("11111")
            # Synchronizes with the simulator and returns the id of the newly
            # started frame (only has effect on synchronous mode).
            world.tick(clock)
            # print(world.tick(clock))
            world.render(display)  # 渲染
            pygame.display.flip()
            control = controller.run_step()
            control.manual_gear_shift = False
            world.player.apply_control(control)
            # transform = world.get_state()

            # print(transform)
            # x, y, z = world.player.get_location()

            # print(world.player.get_velocity())
            # world.vehicle2.apply_control(carla.VehicleControl(throttle=0.6,steer=0.0))
